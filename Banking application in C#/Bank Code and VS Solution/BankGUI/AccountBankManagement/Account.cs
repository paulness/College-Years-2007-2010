...
{
    class Account : AbstractAccount, IAccount
    {
        #region Static Data Members
        static private int m_OverdraftLimit = 1000;
        static private decimal m_IRate = 1.85M; //M is required at the end of literal value to make it assign it as a decimal not double
        #endregion
        #region Data Members
        ...
        //constructor in the case when supplied with just a AccountNumber & customer name
        public Account(string pAccountNumber, string pName)
            : this(pAccountNumber, pName, "")
        {
        }
        //constructor in the case when supplied with AccountNumber, customer name & address
        public Account(string pAccountNumber, string pName, string pAddress)
            : this(pAccountNumber, pName, pAddress, 0) //balance will be set to zero
        {
        }
        //constructor in the case when supplied with AccountNumber, customer name, address, balance
        public Account(string pAccountNumber, string pName, string pAddress, decimal pBalance)
        ...
        //constructor to construct the instance of the Account class when supplied with everything!
        public Account(string pAccountNumber, string pName, string pAddress, decimal pBalance, decimal pOverdraft)
        {
        ...
        //other classes can access m_Balance via the public GetBalance method
        ...
        override public bool SetBalance(decimal pBalance) //private method that gets given pBalance
        {
            if (pBalance >= (-m_Overdraft)) //if balance is not less than the (overdraft as a negative)
            {
                m_Balance = pBalance; //m_Balance gets assigned the value of pBalance
                return true; //SetBalance bool becomes true
            }
            return false; //SetBalance failed & we set the bool to false
        }
        #endregion
        ...
        //Overides the ToString in the parent class (object.ToString())
